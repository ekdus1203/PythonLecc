...
# 아래 처럼은 동시에 책을 읽고, 음악을 듣는 것을 병행할 수 없다
...
# 아래 처럼 1개 함수마다 스레드를 1개 할당해야 동시에 병행 진행할 수 있다
if __name__ == '__main__':
    # 모듈에 스래드 할당을 요청하고, 스레드가 담당할 함수를 지정해주고
    # 스레드의 시작, 대기 등을 제어할 스레드 객체를 리턴 받는다.
    ...
    # 마치 알바생 2명이 각각 맡은 일은 알아서 처리하는 것 처럼 일을 시작한다
    readThread.start()
    playThread.start()
